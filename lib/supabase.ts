...
export const equipmentsApi = {
  getAll: async (): Promise<Equipment[]> => {
    ...
    const { data } = await supabase
      .from('equipments')
      .select('id,name,location,zone,category,status,serial,color,pos_x,pos_y,pos_w,pos_h,food_safe,last_inspection,next_inspection,next_preventive,preventive_interval_days,schema_desc,manual_ref,manufacturer,installation_date,created_at,updated_at,organization_id')
      .order('name')
    return data ?? []
  },
...
getAll: async (): Promise<Part[]> => {
    const { data, error } = await supabase
      .from('parts')
     .select('id,name,ref,category,unit,qty,min_qty,price,supplier,supplier_ref,supplier_contact,supplier2,supplier2_ref,supplier2_contact,location,location_detail,lead_time_days,lead_time_days2,safety_stock,specs,organization_id,created_at,updated_at')
      .order('name')
